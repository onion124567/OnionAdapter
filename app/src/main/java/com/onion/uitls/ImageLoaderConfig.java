...
public class ImageLoaderConfig {
	...
	public static DisplayImageOptions initDisplayOptions(boolean isShowDefault) {
		...
		if (isShowDefault) {
			// 默认显示的图片
//			displayImageOptionsBuilder.showStubImage(R.drawable.banben);
			// 地址为空的默认显示图片
//			displayImageOptionsBuilder
//					.showImageForEmptyUri(R.drawable.banben);
			// 加载失败的显示图片
//			displayImageOptionsBuilder.showImageOnFail(R.drawable.banben);
		}
		// 开启内存缓存
...
//			// 加载失败的显示图片
//			displayImageOptionsBuilder
//					.showImageOnFail(R.drawable.banben);
		}
		// 开启内存缓存
		displayImageOptionsBuilder.cacheInMemory(true);
		// 开启SDCard缓存
